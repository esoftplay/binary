...
<div class="row">
  <div class="col-md-6">
    ...
    <div class="news-phar">
			...
			if(!empty($config['title']))
			{
				...
				{
					?>
					<a href="<?php echo $link;?>" title="<?php echo $data['title'];?>"><h4 style="color:#774500; font-weight: 400"><?php echo $data['title'];?></h4></a>
	        <?php
	      }else{
	      	?>
	      	<h4 style="color:#774500; font-weight: 400"><?php echo $data['title'];?></h4>
	        <?php
	      }
			}
			?>
  ...
  <div class="col-md-6">
		...
		foreach ($cat['list'] as $k => $data)
		{
			$edit_data = (content_posted_permission() && $user->id == $data['created_by']) ? 1 : 0;
			$link      = content_link($data['id'], $data['title']);
			$padding   = $k == 1 ? ' pb-4' : ($k%2 == 0 ? ' py-2' : ' py-4');			
			?>
		    <div class="row<?php echo $padding?>">
		    	...
		    	if (!empty($config['thumbnail']) && !empty($data['image']))
		    	{
			      ...
			      <div class="col-md-<?php echo $col1?>" align="center">
			    		<?php echo content_src($data['image'], ' class="news-img img-responsive" alt="'.$data['title'].'"', false); ?>
			      </div>
		    		<?php
		    	}
		    	$col2 = 12 - $col1;
	...
	if ($config['cat_id'] > 0)
	{
		...
		<div class="row">
		  <div class="col-md-12" align="center" style="margin: 60px 0 0 0;">
		    <a href="<?php echo _URL.'id.htm?cat_id='.$config['cat_id']; ?>" style="color:#774500;"><?php echo lang('See More %s ', @ucwords($cat['title'])) ?><i class="fa fa-arrow-right" aria-hidden="true" style="color: #774500;"></i></a>
		  </div>
		</div>
		<?php
	}
