...
if (empty($id))
{
	...
	$account['serial']  = !empty($account['serial']) ? $account['serial'] : $Bbc->member['username'];
	$account['upline']  = !empty($account['upline']) ? $account['upline'] : lang('none');
	$account['sponsor'] = !empty($account['sponsor']) ? $account['sponsor'] : lang('none');
	...
	if ($exists && !empty($_GET['is_ajax']))
	{
		$output = array(
			'ok'      => 0,
			'message' => 'Sorry, no available data',
			'result'  => array()
			);
		// DEFINE DAYS
		$times  = array();
