...
if (!empty($bundlecat_id))
{
	...
	$list_title = 'Bundle Category';
}

$form->initRoll("WHERE `par_id`={$bundlecat_id} ORDER BY `orderby` ASC");

$form->roll->setSaveTool(true);

$form->roll->addInput('icon', 'file');
$form->roll->input->icon->setTitle('Image');
$form->roll->input->icon->setFolder($Bbc->mod['dir'].'bundle/');
...
$form->roll->onDelete('bin_product_bundle_cat_delete');

function bin_product_bundle_cat_delete($ids)
{
	global $db;
	$ids = bin_product_bundle_cat_recure($ids);
	ids($ids);
	$db->Execute("DELETE FROM `bin_product_bundle_cat` WHERE `id` IN ({$ids})");
}

function bin_product_bundle_cat_recure($ids, $output = array())
{
	global $db;
	if (!empty($ids))
	{
		...
		$r   = $db->getCol("SELECT `id` FROM `bin_product_bundle_cat` WHERE `par_id` IN (".implode(',', $ids).") ");
		if (!empty($r))
		{
