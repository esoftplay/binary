...
if (!empty($_POST['submit_all']))
{
	if (config('plan_a', 'serial_use')=='1')
	{
		$query = "SELECT code, pin, name AS tipe, created FROM `bin_serial` AS s LEFT JOIN `bin_serial_type` AS t ON(s.`type_id`=t.`id`) {$add_sql} ORDER BY s.`type_id` ASC, s.`id` ASC";
	}else{
		$query = "SELECT code, pin, created FROM `bin_serial` AS s LEFT JOIN `bin_serial_type` AS t ON(s.`type_id`=t.`id`) {$add_sql} ORDER BY s.`type_id` ASC, s.`id` ASC";
	}
	if ($_POST['submit_all']=='activate_all')
	{
		$serials = $db->getAll($query);
		$buyer   = @bin_fetch($_POST['buyer_bin_id']);
		$update  = "UPDATE `bin_serial` SET
			`active`       = 1,
		...
		if ($db->Execute($update))
		{
			...
			bin_serial_activate_download($serials, @$buyer['username']);
		}else{
	...
	{
		if (config('plan_a', 'serial_card'))
		{
			$serials = $db->getAll($query);
			_func('download', 'excel', 'serial-'.date('Y-m-d'), $serials);
		}else{
			echo msg('Maaf, anda hanya bisa mendownload serial dengan cara mengaktifkan serial tersebut', 'warning');
		}
	}
}else{
		...
		if (!empty($_POST['roll_activate']) && !empty($_POST['roll_active']))
		{
			...
			if (empty($buyer_bin_id) || empty($_POST['ids']))
			{
				...
				if (!empty($ids))
				{
					if (config('plan_a', 'serial_use')=='1')
					{
						$q = "SELECT code, pin, name AS tipe, created FROM `bin_serial` AS s LEFT JOIN `bin_serial_type` AS t ON(s.`type_id`=t.`id`) WHERE s.`id` IN({$ids}) ORDER BY s.`type_id` ASC, s.`id` ASC";
					}else{
						$q = "SELECT code, pin, created FROM `bin_serial` AS s LEFT JOIN `bin_serial_type` AS t ON(s.`type_id`=t.`id`) WHERE s.`id` IN({$ids}) ORDER BY s.`type_id` ASC, s.`id` ASC";
					}
					$serials = $db->getAll($q);
					bin_serial_activate_download($serials, @$buyer['username']);
...
function bin_serial_activate_download($data, $title, $value='')
{
	$time = date('YmdHis').$value;
	$file = _CACHE.$time.'.json';
	if (file_exists($file))
	{
		call_user_func_array(__FUNCTION__, [$data, $title, $value.'1']);
	}else{
		file_write($file, json_encode($data));
		?>
		<script type="text/javascript">
			setTimeout(function(){
				document.location.href="index.php?mod=bin.serial_activate_download&id=<?php echo $time; ?>&title=<?php echo $title; ?>";
			}, 1000);
		</script>
		<?php
	}
