...
$form->edit->setLanguage('product_id');
$form->edit->setColumn(2);

$form->edit->addInput('header','header');
$form->edit->input->header->setTitle(!empty($id) ? 'Edit Product' : 'Add Product');

$form->edit->addInput('cat_id', 'selecttable');
$form->edit->input->cat_id->setTitle('Category');
$form->edit->input->cat_id->setReferenceTable('bin_product_cat');
$form->edit->input->cat_id->setReferenceField( 'name', 'id' );
$form->edit->input->cat_id->setReferenceNested( 'par_id' );
$form->edit->input->cat_id->setAllowNew(true);

$form->edit->addInput('sku', 'text');
$form->edit->input->sku->setTitle('SKU (Stock Keeping Unit) atau Kode Barang');
$form->edit->input->sku->setRequire('any', true);

$form->edit->addInput('images', 'multifile', 2);
$form->edit->input->images->setFolder($Bbc->mod['dir'].'product/'.$id.'/');
...
$form->edit->addInput('stock', 'text', 2);
$form->edit->input->stock->setTitle('Stock Ketersediaan');
$form->edit->input->stock->setRequire('number', true);
...
$form->edit->input->publish->addTip('Apakah produk ini ingin ditampilkan pada katalog atau tidak?');

$form->edit->addInput('active', 'checkbox', 2);
$form->edit->input->active->setTitle('Availability');
$form->edit->input->active->setCaption('Tersedia');
$form->edit->input->active->setDefaultValue(1);
$form->edit->input->active->addTip('Apakah produk ini tersedia untuk bisa di beli di dalam katalog?');
